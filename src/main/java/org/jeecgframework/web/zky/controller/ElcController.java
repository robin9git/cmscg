...
import org.jeecgframework.web.zky.pojo.base.ElcPubInfo;
...
public class ElcController extends BaseController {
	...
	@ResponseBody
	public AjaxJson save(HttpServletRequest req,ElcPubInfo ent) {
...
//		}
	...
	public void datagrid(ElcPubInfo entity,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(ElcPubInfo.class, dataGrid);
        //查询条件组装器
	...
	public ModelAndView addFiles(ElcPubInfo ent ,HttpServletRequest req) {
		if (StringUtil.isNotEmpty(ent.getId())) {
			ent = systemService.getEntity(ElcPubInfo.class, ent.getId());
	...
	@ResponseBody
	public AjaxJson del(ElcPubInfo ent, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		ent = systemService.getEntity(ElcPubInfo.class, ent.getId());
		//删除内容
