...
import org.jeecgframework.web.zky.pojo.base.Menu;
...
public class MenuController extends BaseController {
	...
	public void datagrid(HttpServletRequest request,
		...
		CriteriaQuery cq = new CriteriaQuery(Menu.class, dataGrid);
	...
	@ResponseBody
	public AjaxJson del(Menu function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		function = systemService.getEntity(Menu.class, function.getId());
	 ...
	 * @param parent
	 */
	private void updateSubFunction(List<Menu> subFunction,Menu  parent) {
		if(subFunction.size() ==0){//没有子菜单是结束
              return;
       }else{
    	   for (Menu tsFunction : subFunction) {
				tsFunction.setFunctionLevel(Short.valueOf(parent.getFunctionLevel()
						+ 1 + ""));
				systemService.saveOrUpdate(tsFunction);
				List<Menu> subFunction1 = systemService.findByProperty(Menu.class, "TSFunction.id", tsFunction.getId());
	...
	@ResponseBody
	public AjaxJson saveFunction(Menu function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		...
		if (function.getTSFunction().getId().equals("")) {
			...
			Menu parent = systemService.getEntity(Menu.class,
		...
		if (StringUtil.isNotEmpty(function.getId())) {
			...
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
			List<Menu> subFunction = systemService.findByProperty(Menu.class, "TSFunction.id", function.getId());
...
//			systemService.flushRoleFunciton(function.getId(), function);
			...
			if (function.getFunctionLevel().equals(Globals.Function_Leave_ONE)) {
				List<Menu> functionList = systemService.findByProperty(
						Menu.class, "functionLevel",
			...
			//设置del_access为1，即新增数据才显示删除按钮，允许删除
			function.setDel_access(Integer.parseInt("1"));
	...
	public ModelAndView addorupdate(Menu function, HttpServletRequest req) {
		String functionid = req.getParameter("id");
		List<Menu> fuinctionlist = systemService
				.getList(Menu.class);
		...
		if (functionid != null) {
			function = systemService.getEntity(Menu.class, functionid);
		...
		if (function.getTSFunction() != null
			...
			function.setTSFunction((Menu) systemService.getEntity(
					Menu.class, function.getTSFunction().getId()));
			req.setAttribute("function", function);
		}
		return new ModelAndView("zky/function/function");
	}
	...
	public List<TreeGrid> functionGrid(HttpServletRequest request,
		...
		if (treegrid.getId() == null) {
			...
			//设置菜单从组织相关开始
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
			
		}
		cq.addOrder("functionOrder", SortDirection.asc);
		cq.add();
		List<Menu> functionList = systemService.getListByCriteriaQuery(cq, false);
		...
		treeGridModel.setType("show_type");
		treeGridModel.setDelAccess("del_access");
	...
	public void functionList(HttpServletRequest request,
		...
		cq.add();
		List<Menu> functionList = systemService.getListByCriteriaQuery(
				cq, false);
		List<TreeGrid> treeGrids = new ArrayList<TreeGrid>();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
...
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
	...
	public ModelAndView searchApp(Menu function, HttpServletRequest req) {
		...
		return new ModelAndView("zky/function/menuAppList");
	}
