...
import org.jeecgframework.web.zky.pojo.base.ZkContentImg;
import org.jeecgframework.web.zky.pojo.base.ZkContentInfo;
...
public class ContentController extends BaseController {
	...
	public ModelAndView addorupdate(ZkContentInfo ent ,HttpServletRequest req) {
		if (StringUtil.isNotEmpty(ent.getId())) {
			ent = systemService.getEntity(ZkContentInfo.class, ent.getId());
	...
	@ResponseBody
	public AjaxJson save(HttpServletRequest req,ZkContentInfo ent) {
	...
	public void datagrid(ZkContentInfo entity,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(ZkContentInfo.class, dataGrid);
        //模糊查询--菜单名称
	...
	@ResponseBody
	public AjaxJson del(ZkContentInfo ent, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		ent = systemService.getEntity(ZkContentInfo.class, ent.getId());
		//删除内容关联的图片
...
//		systemService.executeSql("delete from t_s_document where conId=?", ent.getId());
	...
	public AjaxJson changeStatus(String id, HttpServletRequest req) {
		...
		ZkContentInfo ent = systemService.getEntity(ZkContentInfo.class, id);
	 ...
	 *内容图片相关 
	 * */
	public static void imgDel(SystemService systemService,ZkContentInfo ent){
	 ...
	 *内容图片相关 
	 * */
	public static void imgOpr(SystemService systemService,ZkContentInfo ent){
...
//		systemService.executeSql("delete from zk_content_img where content_id=?", ent.getId());
		...
		if(StringUtil.isNotEmpty(imgUrls)){
			...
			if(arr.length>0){
				ZkContentImg imgEnt = null;
				short s = 1;
				for(int i =0 ;i<arr.length;i++){
					imgEnt = new ZkContentImg();
