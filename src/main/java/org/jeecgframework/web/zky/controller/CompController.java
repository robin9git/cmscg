...
import org.jeecgframework.web.zky.pojo.base.ZkCompInfo;
...
public class CompController extends BaseController {
	...
	public ModelAndView addorupdate(ZkCompInfo ent ,HttpServletRequest req) {
		if (StringUtil.isNotEmpty(ent.getId())) {
			ent = systemService.getEntity(ZkCompInfo.class, ent.getId());
	...
	@ResponseBody
	public AjaxJson save(HttpServletRequest req,ZkCompInfo ent) {
		AjaxJson j = new AjaxJson();
		if (StringUtil.isNotEmpty(ent.getId())) {
			ZkCompInfo ents = systemService.getEntity(ZkCompInfo.class, ent.getId());
	...
	public void datagrid(ZkCompInfo entity,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(ZkCompInfo.class, dataGrid);
