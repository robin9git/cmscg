...
//				}
	...
	public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
		...
		for (Object obj : all) {
			...
			if(Menu.class.isInstance(obj) || MenuEN.class.isInstance(obj)){//如果是menu或menuEN实例，则增加type类型
				String type = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getType()));
				tg.setType(type);
				
				String delAccess = oConvertUtils.getString(reflectHelper.getMethodValue(treeGridModel.getDelAccess()));
				tg.setDelAccess(delAccess);
			}
